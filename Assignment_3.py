...
def ranking_search(query, inverted_index, rank_num):
    # Step 1: Compute IDF values
    N = len(inverted_index)
    idf_values = {}
    for term, doc_ids in inverted_index.items():
        df = len(doc_ids)
        idf_values[term] = math.log(N / (df + 1))  # Add 1 to avoid division by zero

    # Step 2: Create TF-IDF vector for the query
    ...
    for term in query_vector:
        query_vector[term] *= idf_values[term]
    ...
    for term in query_terms:
        if term in inverted_index:
            for doc_id in inverted_index[term]:
                ...
                cosine_similarities[doc_id] += query_vector[term]

    # Normalize cosine similarities by dividing by the magnitude of the query vector
    query_magnitude = math.sqrt(sum(val ** 2 for val in query_vector.values()))
    for doc_id in cosine_similarities:
        doc_magnitude = math.sqrt(sum(val ** 2 for val in query_vector.values()))
        cosine_similarities[doc_id] /= query_magnitude * doc_magnitude
        cosine_similarities[doc_id] = round(cosine_similarities[doc_id],2)

    # Step 4: Rank documents based on similarity scores
    ranked_documents = sorted(cosine_similarities.items(), key=lambda x: x[1], reverse=True)[:rank_num]
...
while(user_input == "Y"):
    ...
    if choice == '1':
        ...
        print("Doc satisfying the query:", result)
    elif choice == '2':
        query = input("Enter search query (multiple words is acceptable) :")
        rank_num = input("Enter number of rankings :")
        ranked_documents = ranking_search(query, inverted_index, int(rank_num))
        print("\nResults for query:", query)
        index = 1
        for doc_id, similarity_score in ranked_documents:
            print(f"{index}. Doc {doc_id}, Similarity Score: {similarity_score}")
            index += 1
    elif choice == '3':
...
#C:/Users/eezhe/Desktop/NTU/Information Retrieval/Assignment_1/output.txt
